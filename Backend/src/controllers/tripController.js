...
const loadTripStopsForRoute = async (tripId, routeId) => {
...
const loadLiveLocationByBusId = async (busId) => {
  const result = await pool.query(
    `
...
export const getTripTimeline = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM trip_full_timeline WHERE trip_id = $1`,
      [tripId]
    );
    ...
    const stops = buildTimelineStops(rawStops, row.bus_lat, row.bus_lng, row.trip_status);
    ...
    return res.status(200).json({
      ...
      bus_lat: row.bus_lat,
      bus_lng: row.bus_lng,
      bus_speed: row.bus_speed,
      bus_updated_at: row.bus_updated_at,
      stops,
      // Backward-compatible aliases for existing UI contracts.
      status: row.trip_status,
      route: row.route_name || `${row.start_point} -> ${row.end_point}`,
      driver: row.driver_name || "Unknown",
    });
  } catch (err) {
    logError("get_timeline_failed", err, { trip_id: Number(tripId) });
...
export const emitTripStopUpdateForBus = async (busId, liveLocationOverride = null) => {
  ...
  const stopRows = await loadTripStopsForRoute(trip.trip_id, trip.route_id);
  const liveLocation =
    liveLocationOverride || (await loadLiveLocationByBusId(trip.bus_id));
  ...
  const allDeparted = Array.isArray(stops) && stops.length > 0 &&
    stops.every((s) => s.state === "departed");
