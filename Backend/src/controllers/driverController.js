...
import { resetBusSmoothing } from "../services/gpsService.js";
import { resetBusEmitState } from "../services/locationTrackingService.js";
import { deleteCache, getCache, setCache } from "../services/cacheService.js";
import { realtimeBus } from "../services/realtimeBus.js";
import { logTripEvent } from "./tripController.js";

const STALE_WARNING_TTL = 600;
const STALE_WARNING_KEY = (tripId) => `stale_warning:${tripId}`;
...
export const startMyTrip = async (req, res) => {
  ...
  try {
    ...
    if (row.inserted_trip) {
      ...
      // BUG2-FIX: Clear smoothing buffers so new trip starts with fresh GPS state
...
export const stopMyTrip = async (req, res) => {
  ...
  try {
    ...
    resetBusEmitState(Number(stoppedTrip.bus_id));
    // Log trip end event
    ...
    realtimeBus.emit("bus:stop", {
      busId: Number(stopped.bus_id),
      trip_id: Number(stopped.id),
      bus_id: Number(stopped.bus_id),
      status: "completed",
      stops: undefined,
    });
    realtimeBus.emit("trip:complete", {
      busId: Number(stopped.bus_id),
      trip_id: Number(stopped.id),
      bus_id: Number(stopped.bus_id),
      status: "completed",
...
export const getMyActiveTrip = async (req, res) => {
  ...
  try {
    ...
    if (stale) {
      ...
      const warnedAtStr = await getCache(STALE_WARNING_KEY(trip.id));
      const warnedAt = warnedAtStr ? parseInt(warnedAtStr, 10) : null;
      const timeSinceWarning = Number.isFinite(warnedAt) ? Date.now() - warnedAt : Infinity;
      ...
      if (timeSinceWarning > 300000) {
        ...
        resetBusEmitState(busId);
        await deleteCache(STALE_WARNING_KEY(trip.id));
        ...
        await setCache(STALE_WARNING_KEY(trip.id), String(Date.now()), STALE_WARNING_TTL);
...
export async function checkAndAutoCompleteStaleTrips() {
  const STALE_MINUTES = Math.max(1, parseInt(process.env.TRIP_STALE_MINUTES ?? "120", 10) || 120);

  const result = await pool.query(
    `
    SELECT t.id AS trip_id, t.bus_id, t.driver_id, ll.updated_at AS last_gps_at
    FROM trips t
    LEFT JOIN live_locations ll ON ll.bus_id = t.bus_id
    WHERE t.status = 'active'
      AND (
        ll.updated_at IS NULL
        OR ll.updated_at < NOW() - ($1::text || ' minutes')::interval
      )
    `,
  ...
  for (const trip of result.rows) {
    ...
    const busId = Number(trip.bus_id);
    if (!Number.isFinite(tripId) || !Number.isFinite(busId)) continue;

    const warnedAtStr = await getCache(STALE_WARNING_KEY(tripId));
    const warnedAt = warnedAtStr ? parseInt(warnedAtStr, 10) : null;

    if (!Number.isFinite(warnedAt)) {
      await setCache(STALE_WARNING_KEY(tripId), String(Date.now()), STALE_WARNING_TTL);
      realtimeBus.emit("bus:stale", {
        busId,
        tripId,
        trip_id: tripId,
      ...
      log("warn", "stale_trip_warning_issued", { trip_id: tripId, bus_id: busId });
      continue;
    }

    const minutesSinceWarning = (Date.now() - warnedAt) / 60000;
    if (minutesSinceWarning <= 5) continue;

    await pool.query(
      `UPDATE trips SET status = 'completed', ended_at = NOW() WHERE id = $1 AND status = 'active'`,
      [tripId]
    );
    await pool.query(
      `UPDATE trip_stops
       SET state = 'departed', departed_at = NOW(), updated_at = NOW()
       WHERE trip_id = $1 AND state <> 'departed'`,
      [tripId]
    );
    await pool.query(`DELETE FROM live_locations WHERE bus_id = $1`, [busId]);
    ...
    resetBusSmoothing(busId);
    resetBusEmitState(busId);
    await deleteCache(STALE_WARNING_KEY(tripId));
    ...
    realtimeBus.emit("trip:complete", {
      ...
      trip_id: tripId,
      bus_id: busId,
      reason: "auto_completed_stale",
    });

    log("warn", "stale_trip_auto_completed", { trip_id: tripId, bus_id: busId });
  }
}
