...
export function validateAndNormalizeGpsPayload(body) {
  const { bus_id, latitude, longitude, speed, accuracy, device_timestamp, sequence, seq } = body;
  ...
  const incomingSequence = sequence ?? seq ?? null;
  let parsedSequence = null;
  if (incomingSequence != null) {
    parsedSequence = Number(incomingSequence);
    if (!Number.isInteger(parsedSequence) || parsedSequence < 0) {
      throw new AppError("Invalid sequence", 400, "INVALID_SEQUENCE");
    }
  }
