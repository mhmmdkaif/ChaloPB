...
import { logTripEvent } from "../controllers/tripController.js";
...
export async function processStopStateMachine(pool, tripId, currentLat, currentLng, hooks = {}) {
  ...
  try {
    ...
    // BUG1-FIX: Lock the row with SKIP LOCKED to prevent concurrent GPS pings
    // from double-committing the same transition. If another request already
    // holds the lock, we exit cleanly with null — not an error.
    const lockResult = await client.query(
      `SELECT id FROM trip_stops WHERE id = $1 FOR UPDATE SKIP LOCKED`,
      [nextStop.id]
    );
    ...
    // Re-read current state AFTER acquiring the lock — another request may
    // have already transitioned this stop between our cache read and now.
    const freshRow = await client.query(
      "SELECT state, arrived_at FROM trip_stops WHERE id = $1",
      [nextStop.id]
    );
    if (freshRow.rows.length === 0 || freshRow.rows[0].state !== fromState) {
      await client.query("ROLLBACK");
      client.release();
      return null;
    }
  ...
  const busId = Number(hooks?.busId);
  if (Number.isFinite(busId)) {
    realtimeBus.emit("bus:stop", {
      ...
      toState: targetState,
      etaMinutes: null,
      stop_order: Number(nextStop.stop_order),
    });
  }
